...
const ReactTablePagination = ({
  ...
  // useEffect(() => {
  //   if (closeVehicleAlarmReducerInfo || isFiltered || isFiltered === '') {
  //     gotoPage(0);
  //     setPageSize(10);
  //   }
  // }, [closeVehicleAlarmReducerInfo, isF\iltered]);
    ...
    <PaginationWrap>
      <Pagination>
        ...
        {arrayPageIndex.map((i) => (
          <Pagination.Item key={i} active={pageIndex === i} onClick={() => gotoPage(i)}>
            {i + 1}
          </Pagination.Item>
        ))}
        <Pagination.Next
      ...
      <PaginationInfo>
        Showing {pageSize * pageIndex + 1} to {pageSize * pageIndex + page.length} of {totalRecords}
      </PaginationInfo>
